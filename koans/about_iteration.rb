...
=end
  ...
  def test_each_is_a_method_on_arrays
    assert_equal true, [:name].methods.include?(as_name(:each))
  end
  ...
  def test_each_can_use_curly_brace_blocks_too
    array = [1, 2, 3]
    sum = 0
    array.each { |item| sum += item } #does the pipelines mean abs value?
    assert_equal 6, sum
  end
  ...
  def test_inject_will_blow_your_mind
    ...
    # Extra Credit:
    # Describe in your own words what inject does. Allow me to input an initial value that I want to block to excute first
  end
